...
#### 与代理软件（v2rayN、Clash 等）共存

`run.ps1` 默认已开启**本地代理绕过阻断**（`-BlockV2rayNProxyBypass $true`），无需额外配置。

当 v2rayN、Clash 等代理软件处于运行状态时，搜狗输入法可能通过以下两种途径绕过防火墙规则：

- **系统代理模式**：输入法连接本机 `127.0.0.1`（或 `::1`）上的代理端口（如 10808、7890 等），由代理进程代为访问网络。
- **TUN 模式**：代理软件创建虚拟网卡，透明拦截所有进程的出站流量。

脚本已针对上述两种模式，为每个搜狗输入法 exe 额外创建两条出站阻断规则：
- 阻断所有目标为 **IPv4 回环地址（127.0.0.0/8）** 的出站连接
- 阻断所有目标为 **IPv6 回环地址（::1）** 的出站连接

无需关心代理软件使用的具体端口或协议，均可有效阻止绕过。

> **注意：** 如已用旧版本的 `run.ps1` 创建了规则，直接重新以管理员身份运行 `run.ps1` 即可——脚本会自动清除同组旧规则并创建更完善的新规则，无需先运行 `uninstall.ps1`。
